...
public class DrawPanel extends JPanel implements Runnable
{
                ...
                if ( isDraggingTool(currentTool) )
                {
                    ...
                    if (( tool.isDrawing()) && (mousePressed==false) )
                    {
                        ...
                        switch(currentTool)
                        {
                            ...
                            case CIRCULE:
                            {
                                elements.add( new CirculeElement (tool.getColor(), ((DragTool) tool).getSPoint(), ((DragTool) tool).getFPoint(),tool.getStroke(),tool.getStrokeStyle()));
                                break;
                            }
                            case SQUARE:
                            {
                            	elements.add( new SquareElement (tool.getColor(), ((DragTool) tool).getSPoint(), ((DragTool) tool).getFPoint(),tool.getStroke(),tool.getStrokeStyle()));
                                break;
                            }
                        }
                    }
    ...
    public void addTemporaryDragElement(Color clr, Point sPoint, Point cPoint, int strokeW, StrokeStyle strokeS)
    {
        switch(currentTool)
        {
            ...
            case CIRCULE:
            {
                elements.add(new CirculeElement(clr, sPoint, cPoint, strokeW, strokeS));
                break;
            }
            case SQUARE :
            {
            	elements.add(new SquareElement(clr, sPoint, cPoint, strokeW, strokeS));
                break;
            }
        }
    }
    ...
    public void setPaintTool(Tool tool)
    {
        ...
        switch (tool)
        {
            ...
            case CIRCULE:
            {
            	this.tool = new CirculeTool(brushColor, this.tool.getStroke());
                Main.paint.paintTools.setToolOptionPanel(new RectShapeToolPanel(Tool.CIRCULE,this.tool.getStroke()));
                break;
            }
            case SQUARE:
            {
            	this.tool = new CirculeTool(brushColor, this.tool.getStroke());
                Main.paint.paintTools.setToolOptionPanel(new RectShapeToolPanel(Tool.SQUARE,this.tool.getStroke()));
                break;
            }
        }
    }
